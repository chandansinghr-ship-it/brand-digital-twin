...
- Bulk-generating AI hero photos for items missing imagery
  (bulk_generate_missing_heroes) — capped at 25 per run
...
const bulkGenerateMissingHeroesTool = defineTool({
  name: "bulk_generate_missing_heroes",
  description: `Generate AI hero photos for menu items that have no primary image and set each as the item's primary photo. Two-step flow: first call without confirm:true to preview the impacted slugs (capped at ${BULK_HERO_CAP}); call again with confirm:true to commit. Photos are flagged is_ai_generated=true and the AI-generated nature must be disclosed when describing them. ALWAYS provide \`reasoning\`.`,
  inputSchema: z.object({
    category: z.string().optional(),
    kitchenLocation: z.string().optional(),
    slugs: z.array(z.string().max(128)).optional(),
  ...
  authScope: "catalog",
  handler: async (input, ctx) => {
    const all = await listItemsMissingPrimary({
      category: input.category,
      kitchenLocation: input.kitchenLocation,
      slugs: input.slugs,
    });
    const targets = all.slice(0, BULK_HERO_CAP);
    if (!input.confirm) {
      return {
        success: true as const,
        preview: {
          matched: targets.length,
          totalAcrossAll: all.length,
          cappedAt: BULK_HERO_CAP,
          cappedAtCap: all.length > BULK_HERO_CAP,
          examples: targets.slice(0, 5).map((t) => ({
            slug: t.slug,
            name: t.name,
            category: t.category,
            kitchenLocation: t.kitchenLocation,
          })),
          slugs: targets.map((t) => t.slug),
        },
        confirmRequired: targets.length > 0,
        aiDisclosure:
          "All photos generated by this run are AI-generated and will be flagged as such.",
      };
    }
    if (targets.length === 0) {
      return {
